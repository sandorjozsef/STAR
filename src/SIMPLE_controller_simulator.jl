function SIMPLE_controller_simulator(patient, simulation)
    ...
    Nutr = 0.0
    if simulation.NutritionDispenser == 1 Nutr=0.25 end
    if simulation.NutritionDispenser == 2 Nutr=0.4 end
    if simulation.NutritionDispenser == 3 Nutr=0.6 end
    patient.P = [patient.P; patient.P[end,1]+selection*60 Nutr]
    
    patient.PN = [patient.PN; patient.PN[end,1]+selection*60 0.0]
